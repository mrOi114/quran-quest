...
- [ ] `npm run typecheck`
- [ ] `npm run lint`
- [ ] `npm run verify:feature-005` (prefs defaults, translation fallback, browse unlock helpers)
- [ ] Apply migration `20260716230000_feature_005_reader_schema.sql` without SQL errors
...
## Guest → account reader prefs merge

- [ ] Guest changes repeat mode / hide meaning / (future) reciter → register → cloud prefs match guest when no prior cloud row
- [ ] Existing cloud prefs are not overwritten by guest values (empty-only merge)
- [ ] Guest browse position migrates only when cloud `learner_reader_state` is empty
- [ ] Font size remains age-derived after register (not a stored preference)
